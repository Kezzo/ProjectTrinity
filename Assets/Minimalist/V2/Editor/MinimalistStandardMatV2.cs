...
public class MinimalistStandardMatV2 : MaterialEditor
{
    #region enums 
    public enum ShadingMode { VertexColor, SolidColor, Gradient }
    ...
    bool ortho;
    Event guiEvent;
    MinimalistClipboard MClipboard;


    public override void OnEnable()
    {
        MClipboard = new MinimalistClipboard();
        base.OnEnable();
    ...
    { 
        ...
        if (targetMat.shader.name != "Minimalist/v2/Standard" && targetMat.shader.name != "Minimalist/v2/LightWeightStandard")
        {
            Repaint();
        ...
        if (MClipboard == null) MClipboard = new MinimalistClipboard();
    ...
    private void TextureModule()
    {
        ...
        if (tex != null)
        {
            ...
            RangeProperty(_MainTexturePower, "Power");
        }
        else
        {
                                    ...
                                    EditorGUI.BeginDisabledGroup(!isAnythingSelected());
                                    {
                                        if (GUILayout.Button(EditorGUIUtility.IconContent("EditCollider", "Edit in Scene"), GUILayout.Height(28)))
                                        {
                                            ...
                                            if (gradientHandle == null)
                                            {
                                                ...
                                                if (ShadingSide == TOP || ShadingSide == DOWN)
                                                {
                                                    gradientHandle.pivot = new Vector3(GradPivot.vectorValue.x, selectedObject.position.y, GradPivot.vectorValue.y);
                                                    gradientHandle.falloff = new Vector3(Gizmopos.vectorValue.z, selectedObject.position.y, Gizmopos.vectorValue.w);

                                                    gradientHandle.pivot = new Vector3(GradPivot.vectorValue.x, selectedObject.position.y, GradPivot.vectorValue.y);
                                                    gradientHandle.falloff = new Vector3(-(GradHeight.floatValue * Mathf.Cos(Rotation.floatValue * Mathf.Deg2Rad)), selectedObject.position.y, -(GradHeight.floatValue * Mathf.Sin(Rotation.floatValue * Mathf.Deg2Rad)));

                                                }
                                                else if (ShadingSide == FRONT || ShadingSide == BACK)
                                                {
                                                    gradientHandle.pivot = new Vector3(GradPivot.vectorValue.x, GradPivot.vectorValue.y, selectedObject.position.z);
                                                    gradientHandle.falloff = new Vector3(-(GradHeight.floatValue * Mathf.Cos(Rotation.floatValue * Mathf.Deg2Rad)), -(GradHeight.floatValue * Mathf.Sin(Rotation.floatValue * Mathf.Deg2Rad)), selectedObject.position.z);

                                                }
                                                else
                                                {
                                                    gradientHandle.pivot = new Vector3(selectedObject.position.x, GradPivot.vectorValue.y, GradPivot.vectorValue.x);
                                                    gradientHandle.falloff = new Vector3(selectedObject.position.x, -(GradHeight.floatValue * Mathf.Cos(Rotation.floatValue * Mathf.Deg2Rad)), -(GradHeight.floatValue * Mathf.Sin(Rotation.floatValue * Mathf.Deg2Rad)));
                                                }
                                        ...
                                        if (GUILayout.Button(EditorGUIUtility.IconContent("Clipboard", "Paste" ), GUILayout.Height(28)))
                                        {
                                            if (MClipboard == null)
                                            {
                                                Debug.LogError("MClipboard is null");
                                            }
                                            else
                                            {
                                                MClipboard.Load();
    ...
    private void AOModule()
    {
        ...
        if (EnableAO)
        {
            ...
            TexturePropertySingleLine(new GUIContent("AO Map"), _AOTexture, _AOPower);
            EditorGUILayout.BeginHorizontal();
            {
                ColorProperty(_AOColor, "AO Color");
    ...
    private void LightmapModule()
    {
        ...
        if (EnableLmap)
        {
            ...
            if (lMapBlendMode == LightMapBlendingMode.UseAsAO)
            {
                ...
                RangeProperty(_LMPower, "Power");
                targetMat.EnableKeyword("LIGHTMAP_AO");
                targetMat.DisableKeyword("LIGHTMAP_ADD");
                targetMat.DisableKeyword("LIGHTMAP_MULTIPLY");
            }
        }
    ...
    {
        ShowGlobalGradientSettings = EditorGUILayout.Foldout(ShowGlobalGradientSettings, "Global Gradient Settings");
    ...
    private void OtherSettings()
    {
        ...
        if (RimEnable)
        {
            targetMat.EnableKeyword("USERIM");
            EditorGUILayout.BeginVertical("box");
            {
        ...
        else { targetMat.DisableKeyword("USERIM"); }

        if (targetMat.shader.name == "Minimalist/v2/LightWeightStandard") {
        ...
        else
        {
            ...
            if (realtimeShadow)
            {
                targetMat.EnableKeyword("SHADOW_ON");
                ColorProperty(_ShadowColor, "Shadow Color");
            }
            else { targetMat.DisableKeyword("SHADOW_ON"); }
        }
        ...
        //gSpace = (GradientSpace)EditorGUILayout.EnumPopup("Shading Mode", gSpace);
    }
    ...
    private void FogModule()
    {
        ...
        if (EnableHFog)
        {
            ...
            targetMat.EnableKeyword("HEIGHT_FOG");
        }
        else { targetMat.DisableKeyword("HEIGHT_FOG"); }
    }
    private void ColorCorrectionModule()
    {
        ...
        {
            targetMat.EnableKeyword("COLORCORRECTION_ON");

            RangeProperty(_Saturation, "Saturation");
            RangeProperty(_Brightness, "Brightness");
            ColorProperty(_TintColor, "Tint");
        }
        else { targetMat.DisableKeyword("COLORCORRECTION_ON"); }
    }
    ...
    public class MinimalistClipboard
    {
        public float falloff;
        public Vector2 pivot;
        public float rotation;

        private const string falloffKey = "Falloff";
        private const string pivotXKey = "PivotX";
        private const string pivotYKey = "PivotY";
        private const string rotationKey = "Rotation";

        public void Save()
        {
            EditorPrefs.SetFloat(falloffKey, falloff);
            EditorPrefs.SetFloat(pivotXKey, pivot.x);
            EditorPrefs.SetFloat(pivotYKey, pivot.y);
            EditorPrefs.SetFloat(rotationKey, rotation);
        }

        public void Load()
        {
            if (EditorPrefs.HasKey(falloffKey)) falloff = EditorPrefs.GetFloat(falloffKey);
            float x = 0, y = 0;
            if (EditorPrefs.HasKey(pivotXKey)) x = EditorPrefs.GetFloat(pivotXKey);
            if (EditorPrefs.HasKey(pivotYKey)) y = EditorPrefs.GetFloat(pivotYKey);
            pivot = new Vector2(x, y);
            if (EditorPrefs.HasKey(rotationKey)) rotation = EditorPrefs.GetFloat(rotationKey);
        }
